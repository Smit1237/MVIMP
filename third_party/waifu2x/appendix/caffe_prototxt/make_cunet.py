...
def unet_conv(bottom, m, o, se=True):
    conv1 = L.Convolution(bottom, kernel_size=3, stride=1, num_output=m, pad=0)
    relu1 = L.ReLU(conv1, in_place=True, negative_slope=0.1)
    conv2 = L.Convolution(relu1, kernel_size=3, stride=1, num_output=o, pad=0)
...
def unet_branch(bottom, insert_f, i, o, depad):
    ...
    feat = insert_f(relu1)
    unpool = L.Deconvolution(
        feat, convolution_param=dict(num_output=o, kernel_size=2, pad=0, stride=2)
    )
...
def unet1(bottom, ch, deconv):
    ...
    if deconv:
        return L.Deconvolution(
            conv2, convolution_param=dict(num_output=ch, kernel_size=4, pad=3, stride=2)
        )
    else:
